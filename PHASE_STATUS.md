...
Database schema complete. Backend authentication (endpoints, bcrypt, JWT cookies) complete. Remaining: role middleware, frontend auth pages, protected routes.
...
**Phase 1 — Step 3: Role-based middleware (student vs admin route protection)**

Add FastAPI dependencies to extract the current user from the access-token cookie and enforce role checks on protected routes (admin-only vs authenticated-user).

**Reference docs:**

1. `docs/02_Architecture.md` — Section 4.4 Auth Rules
2. `docs/10_BuildPhases.md` — Phase 1 prompt

**Prerequisite:** Review and approve backend authentication implementation.
